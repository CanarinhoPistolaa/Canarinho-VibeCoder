 ...
 *
 * canarinho is the main tool used to develop canarinho itself, so a test (or
 * anything a test spawns) touching the REAL ~/.canarinho state or binding
 ...
 * happened. `npm test` sets canarinho_TEST_GUARD=1 (passed through
 ...
 * suite shelling out to the canarinho CLI), set canarinho_TEST_GUARD=0.
 */
import os from "node:os";
import path from "node:path";

/** Ports the live canarinho instance uses by default. */
const PRODUCTION_PORTS = new Set([3334, 3338, 3339]);

export function testGuardActive(): boolean {
  // canarinho_TEST_GUARD=0 is an explicit escape hatch that disables the
  // guard even when NODE_TEST_CONTEXT is set (e.g. third-party test suites
  // that shell out to the canarinho CLI).
  if (process.env.canarinho_TEST_GUARD === "0") return false;
  ...
  if (process.env.canarinho_TEST_GUARD === "1") return true;
...
export function assertPortIsolation(port: number, what: string): void {
  ...
  throw new Error(
    `TEST ISOLATION VIOLATION: ${what} tried to bind production port ${port} while ` +
      `canarinho_TEST_GUARD=1. Tests (and anything they spawn) must use random ports — ` +
      `see reservePortHandle() in tests/helpers/test-env.ts and set ` +
      `canarinho_CONTROL_PORT / explicit port arguments for every daemon they start.`,
  );
}

/** Throw if a state path resolves into the REAL ~/.canarinho under the guard. */
export function assertStatePathIsolation(resolvedPath: string, what: string): void {
  if (!testGuardActive()) return;
  const home = realUserHome();
  if (!home) return;
  const realStateDir = path.join(home, ".canarinho");
  const normalized = path.resolve(resolvedPath);
  if (normalized === realStateDir || normalized.startsWith(realStateDir + path.sep)) {
    throw new Error(
      `TEST ISOLATION VIOLATION: ${what} resolved to the real canarinho state ` +
        `(${normalized}) while canarinho_TEST_GUARD=1. Tests must point HOME / ` +
        `canarinho_STATE_DIR / canarinho_DB_PATH at a per-test temp directory.`,
    );
  }
}
